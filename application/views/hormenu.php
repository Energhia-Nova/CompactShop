<div class="nav">
	<div class="icon"><a href="/" style="font-size: 20px; font-family: Verdana; color: white; font-weight: bold; text-shadow: 1px 1px 3px grey;">Магазин</a></div>
	<?php foreach($pages as $iter=>$page):?>
		<div class="nav_menu">
			<?php if (!array_key_exists('items',$page)) { 
					
					if ($page['alias']=='face' and Auth::instance()->logged_in())
						echo '<p class="bold link"><a href="/exit">Выйти.</a></p>';
					else
					{
						if ($page['alias']=='sign-up' and Auth::instance()->logged_in())
						{
							echo '<p class="bold link"><a href="#" id="total_price">'.$total_price.'</a> грн.</p> <a href="/order" class="R" title="Заказ заполнен">R</a>';
						}
						else 
